...
create policy "users own campaign_recipients" on campaign_recipients for all using (exists (select 1 from campaigns where campaigns.id=campaign_recipients.campaign_id and campaigns.user_id=auth.uid())) with check (exists (select 1 from campaigns where campaigns.id=campaign_recipients.campaign_id and campaigns.user_id=auth.uid()));
create policy "users own email_threads" on email_threads for all using (auth.uid()=user_id) with check (auth.uid()=user_id);
create policy "users own email_messages" on email_messages for all using (exists (select 1 from email_threads where email_threads.id=email_messages.thread_id and email_threads.user_id=auth.uid())) with check (exists (select 1 from email_threads where email_threads.id=email_messages.thread_id and email_threads.user_id=auth.uid()));
create policy "users own email_templates" on email_templates for all using (auth.uid()=user_id) with check (auth.uid()=user_id);
create policy "users own ai_prompts" on ai_prompts for all using (auth.uid()=user_id) with check (auth.uid()=user_id);
-- Additional tables with user_id
...
create policy "users own tags" on tags for all using (auth.uid()=user_id) with check (auth.uid()=user_id);
create policy "users own integrations" on integrations for all using (auth.uid()=user_id) with check (auth.uid()=user_id);
