...
export default async function CampaignDetail({params}:{params:Promise<{id:string}>}){
  ...
  const { data: recips } = await supabase.from('campaign_recipients').select('*').eq('campaign_id',id).limit(50)
  const list=recips||[{id:'1', recipient_email:'info@abcdental.com', status:'QUEUED'},{id:'2', recipient_email:'hello@miami.com', status:'SENT'}]
  ...
  return <AppShell><h1 className="text-2xl font-bold">Campaign {id}</h1><p className="text-sm text-zinc-500">Queue: QUEUED → SENDING → SENT with rate-limit. Follow-ups Day 0 / Day 3 / Day 7. Auto-stop on REPLIED/UNSUBSCRIBED/WON.</p>
    ...
    <Card className="mt-6">
      ...
      <p className="text-xs text-zinc-500 mt-1">Default sends only to VERIFIED. 112 verified • 21 risky • 17 invalid (example). Stop if lead becomes UNSUBSCRIBED/NOT_INTERESTED/WON.</p>
    ...
    <Card className="mt-6"><h3 className="font-semibold">Follow-ups</h3><div className="flex gap-2 mt-3 text-sm"><span className="px-3 py-1 border rounded-full">Day 0: Initial</span><span className="px-3 py-1 border rounded-full">Day 3: Follow-up</span><span className="px-3 py-1 border rounded-full">Day 7: Second follow-up</span></div><p className="text-xs text-zinc-500 mt-2">If reply detected → STOP. Stored in followups table.</p></Card>
  </AppShell>
}
