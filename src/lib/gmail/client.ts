...
export function getOAuth2Client(tokens:{access_token?:string; refresh_token?:string; expiry_date?: string}){
...
export async function sendEmailViaGmail(tokens:any, to:string, subject:string, body:string, opts:{cc?:string; bcc?:string}={}){
  ...
  const headers=[`To: ${to}`, opts.cc?`Cc: ${opts.cc}`:'', opts.bcc?`Bcc: ${opts.bcc}`:'', `Subject: ${subject}`, 'Content-Type: text/html; charset=utf-8',''].filter(Boolean).join('\n')
...
export async function getMessage(tokens:any, id:string){
  const auth=getOAuth2Client(tokens); const gmail=google.gmail({version:'v1', auth}); const res=await gmail.users.messages.get({userId:'me', id}); return res.data
}
