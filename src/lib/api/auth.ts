...
export interface ResolvedAuth { userId: string|null; via: 'session'|'extension_token'|null; error?: string }
...
export async function resolveUser(req: Request): Promise<{ auth: ResolvedAuth; supabase: any }> {
  ...
  try{
    ...
    if(user?.id) return { auth:{ userId:user.id, via:'session' }, supabase }
  }catch{}
